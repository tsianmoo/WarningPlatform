...
const LS_KEY_BASE = 'alert-platform-v1';

/**
 * 本地缓存键按登录账号隔离。
 *
 * 旧版是全局单键 `alert-platform-v1`：多人共用一台电脑时，后登录的人
 * 会先看到上一个人的缓存数据（服务端请求返回前的那一帧，或者请求失败时一直看到）。
 * 现在以账号 id 作为后缀，各账号互不可见。
 */
function lsKey(): string {
  try {
    const id = localStorage.getItem('dn_account_id');
    return id ? `${LS_KEY_BASE}::${id}` : LS_KEY_BASE;
  } catch {
    return LS_KEY_BASE;
  }
}
...
/**
 * 会话失效处理：清掉本地登录标记并回登录页。
 *
 * 旧版这里是静默失败：接口 401 就退回 localStorage，用户界面照常能用、
 * 但所有改动都写不进数据库（下次刷新全没了），属于最容易被误判成「数据丢失」的坑。
 */
 ...
 * 显式删除通道。
 *
 * 服务端已改为「只 upsert、绝不按入参删除」，因此每次删除都必须在这里显式声明目标 id。
 * 旧版依赖「客户端快照全量覆盖」顺带完成删除，一旦某次快照不完整就会误删其它人的数据。
 */
interface RemoteDeletions {
  ruleIds?: string[];
  groupIds?: string[];
  tableIds?: string[];
  ...
  alertIds?: string[];
  clearAlerts?: boolean;
  /** 被移除的角色（按主体类型 + 标识定位，roles 表是复合主键，没有单列 id） */
  roleKeys?: { post: string; subjectKind?: string }[];
  /** 被移除的单人权限覆盖 */
  overridePersonIds?: string[];
}

async function deleteRemote(drop: RemoteDeletions): Promise<void> {
  const keys = Object.keys(drop).filter((k) => k !== 'clearAlerts') as (keyof RemoteDeletions)[];
  if (keys.length === 0) return;
  try {
    const res = await fetch(STATE_API, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ del: drop }),
    });
    if (res.status === 401) {
      remoteAvailable = false;
      handleSessionExpired();
      ...
      console.warn('[persist] 权限不足，未删除:', j?.error ?? res.status);
    }
  } catch {
    /* 删除失败不阻塞前台操作，用户下次进入时会看到该项仍在 */
  }
}
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        ...
        const j = (await res.json().catch(() => null)) as { error?: string } | null;
        console.warn('[persist] 权限不足，本次未写入:', j?.error ?? res.status);
      } else {
...
function reducer(state: AppState, action: { type: string; payload?: unknown }): AppState {
  switch (action.type) {
    ...
    case 'SET_PERMISSIONS': {
      const next = Array.isArray(action.payload) ? (action.payload as RolePerm[]) : state.permissions;
      // 角色走「以本次提交为准」：算出被移除的角色，显式通知服务端清理。
      // 不这样做的话，删掉的角色仍留在库里继续生效（只 upsert 不删除的副作用）。
      const kept = new Set(next.map((p) => `${p.post}\u0000${p.subjectKind ?? 'post'}`));
      const removed = (state.permissions ?? [])
        .filter((p) => !kept.has(`${p.post}\u0000${p.subjectKind ?? 'post'}`))
        .map((p) => ({ post: p.post, subjectKind: p.subjectKind ?? 'post' }));
      if (removed.length > 0) void deleteRemote({ roleKeys: removed });
      return { ...state, permissions: next };
    }
    case 'SET_PERM_OVERRIDES': {
      const next = Array.isArray(action.payload) ? (action.payload as PersonPermOverride[]) : state.permOverrides;
      const kept = new Set(next.map((o) => String(o.personId ?? '')).filter(Boolean));
      const removed = (state.permOverrides ?? [])
        .map((o) => String(o.personId ?? ''))
        .filter((id) => id && !kept.has(id));
      if (removed.length > 0) void deleteRemote({ overridePersonIds: removed });
      return { ...state, permOverrides: next };
    }
    case 'RESET':
      return loadInitialState();
    default:
  ...
  const api = useMemo<StoreApi>(() => {
    ...
    return {
      ...
      removeTable: (id) => {
        ...
        void deleteRemote({ tableIds: [id] });
      },
      saveTableRows: async (id, rows) => {
        // 大表（数万行）一次性整包 POST 容易因请求体过大/超时静默失败（浅秋零售单 7.6 万行就是这样丢的），
        // 超过阈值时改为分片：第一片 start（清空重写），后续片 append，单片失败重试 3 次。
        ...
        try {
          if (all.length <= CHUNK) {
            return await post({ rows: all });
          }
          for (let i = 0; i < all.length; i += CHUNK) {
            const phase = i === 0 ? 'start' : 'append';
            let ok = false;
            for (let attempt = 0; attempt < 3 && !ok; attempt++) {
              try {
                ok = await post({ rows: all.slice(i, i + CHUNK), phase, offset: i });
              } catch {
                ok = false;
              }
              if (!ok) await new Promise((r) => setTimeout(r, 500 * (attempt + 1)));
            }
            if (!ok) return false; // 某片彻底失败：返回 false，前端提示重新上传
          }
          return true;
        } catch {
          return false;
      ...
      removeTableGroup: (id) => {
        dispatch('REMOVE_TABLE_GROUP', id);
        void deleteRemote({ tableGroupIds: [id] });
      },
      ...
      removeOrg: (id) => {
        dispatch('REMOVE_ORG', id);
        void deleteRemote({ orgIds: [id] });
      },
      ...
      removePerson: (id) => {
        ...
        void deleteRemote({ personIds: [id] });
      },
      addHrAttribute: (a) => {
      ...
      removeHrAttribute: (id) => {
        dispatch('REMOVE_HRATTR', id);
        void deleteRemote({ hrAttributeIds: [id] });
      },
      addDealer: (d) => {
