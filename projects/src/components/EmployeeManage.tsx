...
export default function EmployeeManage({ onBack }: { onBack: () => void }) {
  ...
  const save = () => {
    ...
    const data = { code: c, name: n, dealerId: form.dealerId || undefined, storeId: form.storeId || undefined, post: form.post || undefined, onDuty: form.onDuty, enabled: form.enabled, password: form.password || undefined };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setEditing(null)}>
          <div className="w-[420px] rounded-lg bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className={label}>岗位</label><input value={form.post} onChange={(e) => setForm({ ...form, post: e.target.value })} className={input} /></div>
