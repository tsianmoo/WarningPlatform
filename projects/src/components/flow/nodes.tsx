...
function TargetPanel({ targets, onChange }: { targets: TargetSetting; onChange: (t: TargetSetting) => void }) {
  ...
  const togglePersonPost = (p: string) =>
    onChange({
      ...targets,
      personPosts: targets.personPosts?.includes(p)
        ? targets.personPosts.filter((x) => x !== p)
        : [...(targets.personPosts ?? []), p],
    });
  const { state } = useStore();
    ...
    <div className="mt-1.5 rounded-lg border border-amber-200/70 bg-amber-50/50 p-1.5">
      ...
      {mode === 'person' && (
        <div className="space-y-1.5">
          ...
          <div>
            <div className={groupLabel}>按职位筛选推送用户（可不选）</div>
            <div className="flex flex-wrap gap-1">
              {positions.length === 0 && (
            ...
            <div className="mt-1 text-[10px] text-gray-400">职位、岗位可不选；均不选 = 推送范围内全部用户，选中则仅推送给匹配的用户</div>
          </div>
          <div className="mt-1.5">
            <div className={groupLabel}>按岗位筛选推送用户（可不选）</div>
            <div className="flex flex-wrap gap-1">
              {posts.length === 0 && (
                <span className="text-[10px] text-gray-400">暂无岗位，请先在「用户管理」为用户设置岗位</span>
              )}
              {posts.map((post) => {
                const on = targets.personPosts?.includes(post) ?? false;
                return (
                  <button
                    key={post}
                    type="button"
                    onClick={() => togglePersonPost(post)}
                    className={`rounded px-1.5 py-0.5 text-[10px] transition ${
                      on ? 'bg-sky-500 text-white' : 'bg-white text-gray-500 hover:bg-sky-100'
                    }`}
                  >
...
const RowSortNode = memo(function RowSortNode({ id, data }: NodeProps) {
  ...
  const addPivot = () => {
    const primary = merged.find((c) => c.show !== false);
    const numCol = merged.find((c) => /数量|库存|金额|求和|sales|qty|val|amount|sum/i.test((c.key || c.label || ''))) || merged[merged.length - 1];
    const rowCol = primary && numCol && (primary !== numCol) ? primary : merged.find((c) => c !== numCol) || primary;
    update({
      pivots: [
        ...pivots,
        {
          ...
          valueField: numCol ? numCol.label || numCol.key : '',
          enable: true,
        } as RowSortPivot,
      ],
    } as Partial<RowSortNodeData>);
  ...
  return (
    <NodeShell fnode={fnode}>
      <div className="space-y-1.5">
        ...
        {cols.length === 0 && <div className="text-[10px] text-gray-400">选择节点结果后自动带出全部字段，可在此调整顺序与格式。如需多指标横向并列展示，请在下方「③ 多横排块」添加。</div>}
          ...
          <div className="space-y-1.5">
            ...
            {activePivotCount > 0 && (
              <div className="rounded-md border border-violet-100 bg-violet-50/50 px-2 py-1 text-[10px] leading-4 text-violet-600">
                已启用 {activePivotCount} 个横排块：每个块按自身「行转列字段」横向展开，并以其「值字段」填充数值，与②固定列并列输出。
              </div>
            )}
          </div>
        </div>
...
const LinkViewNode = memo(function LinkViewNode({ id, data }: NodeProps) {
  ...
  // 基础上表：预警动作节点结果（其字段作为基础表字段，可改选其它节点结果）
    ...
    <NodeShell fnode={fnode}>
      <div className="space-y-1.5">
        <div className="mb-1 rounded-md bg-pink-50 px-2 py-1 text-[10px] leading-relaxed text-pink-700">
          以预警动作结果为基础上表，选择关联数据表/节点并配置「基础表字段 ↔ 关联表字段」匹配键，弹窗内只展示与本预警对应结果匹配的关联数据（如手机号 115 → 返回 115 的资料）。
        </div>
        {/* 基础上表 */}
        <div className="rounded-md border border-gray-100 bg-gray-50/60 p-1.5">
          <div className="mb-1 text-[11px] font-medium text-gray-600">基础表（预警动作结果，展示其字段）</div>
          <select
            value={baseNodeId}
          ...
          return (
            <div key={i} className="rounded-md border border-gray-100 bg-gray-50/60 p-1.5">
              <div className="flex items-center gap-1">
                ...
                <input value={tab.name ?? ''} onChange={(e) => setTab(i, { name: e.target.value })} placeholder="如：会员资料" className={`${inputCls} flex-1`} />
                <button type="button" onClick={() => removeTab(i)} className="shrink-0 text-[10px] text-red-400 hover:text-red-600">删</button>
              </div>
              <div className={rowLabel}>关联来源</div>
              <div className="flex gap-1">
              ...
              {keys.length === 0 && <div className="mb-1 rounded bg-amber-50 px-2 py-1 text-[10px] text-amber-700">未配置匹配字段时，展示该关联来源的全部行。</div>}
              {keys.map((kk, ki) => (
        ...
        {tabs.length > 0 && (
          <button type="button" onClick={addTab} className="w-full rounded-md border border-dashed border-pink-300 py-1 text-xs text-pink-600 hover:bg-pink-50">+ 添加关联标签</button>
        )}
      </div>
    </NodeShell>
  );
