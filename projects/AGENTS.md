...
- **通知对象 = 独立流程节点（kind `notify`，v2025.13 起）**：用户要求"通知对象与预警动作分开、做成独立流程节点"。通知对象节点 `NotifyNodeData { mode, stores[], staff[], departments[], custom[] }`，`mode` 单选：**`store` 按门店**（激活后被命中的每个门店各单独收一条）/ **`staff` 按门店员工**（每个门店的导购收与自己相关的预警）/ **`dept` 按部门**（门店所负责部门收）/ **`custom` 自定义**（指定的人收）。`stores/staff/departments/custom` 对应各自的多选下标（staff/custom 选自 `PERSONNEL[].name`，store 选自 `STORES`，dept 选自 `DEPARTMENTS`）。注册链路：`NodeKind + 'notify'` → `NotifyNode` 组件（nodes.tsx，粉色/粉红风格 `#DB2777`，`NodeShell` + 顶部 title + mode 单选按钮 + 多选 chips + 左 Handle）→ `createNodeData` 默认 `{mode:'store',...}` → `nodeTypes.notify` → FlowCanvas palette 由 `kind:'notify'` 入口拖入（色 `KIND_COLOR.notify`）→ `KIND_ICON`/`nodeKindCn`/`KIND_LABEL` 同步补 notify。`AnyData` union 需加 `NotifyNodeData`（否则 `useNodeUpdater` 的 update patch 类型报 mode 非法）。evaluate 对 notify 走 default 兜底（终点配置节点无数据输出）。**action 与 notify 关联**：`ActionNodeData` 删除内嵌 TargetPanel，改为 `refNotifyNode?: {nodeId,label}`——action 节点 UI 提供「通知对象（独立节点）」下拉，从画布上 `kind==='notify'` 节点选一个（option 显示 `mode 标签（已选 N 项）`）；未选则回退旧 `notify` 字段。`buildAlertsForRule`（store.tsx）通过 `action.refNotifyNode.nodeId` 在 `rule.flow.nodes` 找 notify 节点读策略：`dept` = dept 模式 departments.join('、')、store 模式 stores.join('、')、否则回退旧 notify；`assignee` = staff 模式 staff.join('、')、custom 模式 custom.join('、')、否则回退旧 notify。诊断：esbuild 打包 `buildAlertsForRule`（jsx automatic，external react/react-jsx-runtime）跑 mode=staff → assignee="张伟、李娜" 通过。旧 `TargetSetting`/`TargetPanel` 已移除（TargetPanel 不再存在于 nodes.tsx，其符号删净避免未使用警告）。
