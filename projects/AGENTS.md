...
- **规则配置页（RuleConfigurator）顶栏**：左侧 = 返回 + 标题输入 + 标题右侧的规则分组下拉（读 `state.ruleGroups`，改 `rule.groupId`，`''`=未分组）；右侧 `ml-auto` 右对齐 = 「流程搭建步骤条（STEPS）」+「保存草稿 / 激活规则」。新建规则引导 NewRuleGate 已支持选/新建分组（ADD_RULE_GROUP reducer + StoreApi.addRuleGroup 返回建好 id 的对象立即选中）。
