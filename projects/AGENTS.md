...
- **baseline（基准统计）支持分组维度 dims（可多字段）**：`BaselineNodeData.dims` 为 `{key,label}[]`（空=对全部取值求一个全局基准）。UI 配置顺序：①引用节点输出/数值字段 → ②分组维度（可多字段，`+ 添加维度字段`，维度列取 `inferNodeCols(refNode)` 的列或数据表字段）→ ③统计方式 → ④结果命名。**命名上标题机制已内置**：`nodeTitle` 对 `baseline`/`groupby` 返回 `基准统计（resultLabel）` / `分组聚合（resultLabel）`（resultLabel 非空时），无需额外改 NodeShell。evaluate baseline 分支：`dims` 非空 → 按维度分组逐行求统计（`columns=[维度label..., resultLabel]`，`shape:'table'`，无 scalar）；`dims` 空 → 保持单行 scalar（`scalar:{label,value}`，供判断组件右值选节点，行为与 groupby 空维度一致）。改 evaluate baseline 输出列时同步 inferNodeCols（新增维度列会让下游可引用列变化）。
- **分组维度候选要"穿透上游数据表字段"才完整**：仅 `inferNodeCols(refNode)` 只会给到 refNode 输出列（如"未开单天数+店仓名称"），**缺上游源表的业务维度列**（分公司/销售区域2/区部2/主营品牌等）。修复：dimsOptions 合并收集 refId 链上各节点的 `data.tableId/factTableId/universeTableId` 对应源表的全部 `fields`（label 取 `f.alias||f.label||f.key`），再叠加 inferNodeCols(refNode) 去重。**BFS 遍历上游 nodeId 引用时必须兼容两种形态**：filljoin/compute 用 `data.expr.ref/.left/.right`（compute 双节点运算是**对象** `{nodeId,col}`，外层又有 `expr.left` 对象），不能只按数组 `expr.left?.[0]` 取——否则回调到 filljoin 就断层、拿不到其 universeNodeId(如 filter) 的源表字段。遍历键需含字符串键 `universeNodeId/factNodeId/sourceNode/aggNode/leftNodeId/rightNodeId` + `expr` 内的对象/数组 ref。
