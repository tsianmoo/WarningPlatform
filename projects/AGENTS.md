...
- **组织架构 / 人事架构**：承载预警通知对象（最终通知到"用户和人员"）。组织 `Organization`（表 `organizations`：`id/name/kind/parent_id/sort/created_at`）——用户明确"是平级标签"（总部/分公司/部门/区域/门店/其他，`kind` 分类，`parentId` 预留但默认不同）。人员 `Person`（表 `persons`：`id/name/org_id/title/post/supervisor_id/manage_scope(JSONB)/phone/email/enabled/sort/created_at`，`title`=职位、`post`=岗位，均来自「职位/岗位管理」属性标签），**挂在组织节点下**（`orgId`）；`manageScope` 用 `ManageScope{tableId,field,value,desc}` 表达"管理范围＝某数据表某字段=某分类值"（如 该店长管理"大区=华东大区"下的门店/人员）。`supervisorId` 为上级人员（空=最高层）。经 `/api/state` 的 `orgs`/`persons` 全量同步（repo `getAllOrgs/syncOrgs/getAllPersons/syncPersons`）。页面：`OrgArch.tsx`（组织分类）、`PeopleManage.tsx`（人员管理：部门树+人员增删改；部门「编辑」复用 `OrgModal` 弹窗，可改名称/上级部门/分类）、`AttrManage.tsx`（属性管理：属性+子标签），挂载于 `page.tsx` 的 `View='org'/'people'/'attrs'`（侧边导航「人事管理」父级下展开 人员管理/属性管理）。通知动作面板（`nodes.tsx` TargetPanel）通知对象已改为从 `state.persons`/`state.organizations` 动态选择（`notify.orgIds`/`personIds`），不再用写死常量 `PERSONNEL`/`DEPARTMENTS`（import 已移除）。
  ...
  - **首页管理画布式编辑器（当前版 `HomeConfig.tsx`）**：布局改为「顶部配置栏 + 下方画布」——所有配置控件集中在顶部（元素选择 chips「背景/登录框/主标题/副标题」+ 通过组件添加的文本/图片），下方为登录页实时预览画布；顶栏「组件」按钮可**添加文本 / 图片元素到画布**（文本 `HomeTextElement`、图片 `HomeImageElement`，复用 `HomeElement` 联合类型，新增字段 `HomeConfig.elements: HomeElement[]`，见 `src/lib/types.ts`，`uid()` 生成 id）；点击画布中任一组件（或顶部 chip）将其选中，顶部配置栏随之切换显示该组件的样式控件并带删除按钮；未选中时点画布空白区可取消选中。拖动统一走 `startDrag(key,e)/setPos(key,x,y)` + `dragRef` + window pointer 监听，移动 `login/title/subtitle/elem:<id>` 的 `x/y%`（元素用 `translate(-50%,-50%)` 以元素**中心点**定位，登录页渲染逻辑与之一致）。新增元素由「组件→添加图片」走 `addImageFileRef`/`onAddImageFile`（总是新增）、已选图片元素「上传/更换图片」走 `imageFileRef`/`onImageFile`（`selectedElemId` 命中则替换 src）——**两个文件入口刻意分离**，避免"选中图片时从菜单添加却被误判为替换"。改 `HomeConfig` 元素渲染/定位时须同步 `login/page.tsx` 的 `cfg.elements` 渲染段（文本 `translate(-50%,-50%)` + 不换行；图片 `object-cover` + `width/height/borderRadius/opacity`）。
