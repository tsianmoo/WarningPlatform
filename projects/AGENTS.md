...
- **基础数据（base）取列模式**：`BaseNodeData` 支持 `columns:{key,label}[]` 多选输出列，`distinct` 去重仅当勾选**单列**时生效（多列强制不去重）。无 `columns` 时回退旧 `fieldKey/fieldLabel` 单列去重。evaluate 的 base 分支按 `source!=='node' && bd.columns` 决定取列表格输出；col `columns` 只在数据表模式生效，切到节点模式会回退 fieldKey。inferNodeCols/getNodeOutputs 的 base 分支均按 selected columns 输出下游可选列。
