...
- **通知对象 = 独立流程节点（kind `notify`）**：用户要求"通知对象设为独立流程节点、与预警动作分开"，**各投递维度独立多选、接收人取并集**（可同时通知门店 + 门店员工 + 管理部门 + 部门职位 + 自定义指定的人）。`NotifyNodeData { stores[], staff[], departments[], positions[], custom[], notifyLabel? }`（**无 mode 单选**）：`stores` 按门店（对应门店收到该店预警消息）、`staff` 按门店员工（门店导购收与自己相关的预警）、`departments` 按管理部门、`positions` 按部门职位、`custom` 自定义指定的人（staff/custom 选自 `PERSONNEL[].name`，store 选自 `STORES`，dept 选自 `DEPARTMENTS`，position 选自 `POSITIONS`）。注册链路：`NodeKind + 'notify'` → `NotifyNode`（辅导 pink 风格 `#DB2777`，顶部 `notifyLabel||"通知对象（N）"` + 5 组可独立勾选 chips，每组右上角已选计数，底部"各维度同时通知，接收人取并集"提示 + 左 Handle）→ `createNodeData` 默认五维空数组 → `nodeTypes.notify` → FlowCanvas palette `kind:'notify'` 拖入 → `KIND_ICON/KIND_LABEL` 补 notify。`AnyData` union 含 `NotifyNodeData`。evaluate 对 notify 走 default 兜底（终点配置节点无数据输出）。**action 与 notify 关联**：`ActionNodeData.refNotifyNode?: {nodeId,label}`，action 目标区「通知对象（独立节点）」下拉从画布选 notify 节点（option 与已关联 label 显示 `notifyLabel || "通知对象（N项）"`）；未选回退旧 `notify` 字段。`buildAlertsForRule`（store.tsx）由 `refNotifyNode.nodeId` 在 `rule.flow.nodes` 找 notify 节点：`dept` = departments.concat(positions).join('、')、`assignee` = staff.concat(custom).join('、')，为空回退旧 notify。诊断 esbuild（jsx automatic，external react）跑 5 维并集 → dept="风控部、经理"、assignee="张伟、王强" 通过。`TargetSetting`/`TargetPanel` 已移除。
